...
void sbrkfail(char *s)
{
        ...
        for (i = 0; i < sizeof(pids) / sizeof(pids[0]); i++)
        {
                if (pids[i] == -1)
                {
                        printf("failed to fork PID[%d]: %d\n", i, pids[i]);
                        continue;
                }
                kill(pids[i]);
                wait(0);
        }
        // Need to wait until all these processes were killed.
        // wait just collects the zombie processes
                // And then sleeps so that the child process can call exit(-1) and then
        if (c == (char *)0xffffffffffffffffL)
        {
